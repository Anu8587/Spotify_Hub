    ...
    beat_schedule={
        "send-weekly-newsletter": {
            ...
            # "schedule": crontab(minute="*"),  # Monday at 9 AM
             'schedule': crontab(hour=10, minute=38, day_of_week=6),  # Monday at 10 AM
        },
        "fetch-all-users-data-daily": {
            "task": "tasks.daily_fetch_task",
            "schedule": crontab(hour=10, minute=30),  # Run daily at midnight UTC
        },
        "update-weekly-stats": {
            "task": "tasks.weekly_update_task",
            "schedule": crontab(
                hour=10, minute=34, day_of_week=6
